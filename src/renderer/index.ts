...
function shell(content: string): string {
  ...
  return `<div class="shell">${top}<div class="app-layout">${nav()}<main id="main" class="main" tabindex="-1">${warning}${content}</main></div></div>`;
}
...
function preview(showGuide = true): string {
  ...
  return `<div class="preview" aria-label="Local camera preview area"><video id="camera-preview" autoplay muted playsinline aria-label="Mirrored local camera preview"></video>${previewStream ? '' : personArt()}${showGuide ? '<span class="preview-label"><span class="dot"></span>Camera on · local preview</span>' : ''}</div>`;
}
...
function positioning(): string {
  ...
  return `${heading('Position', 'Set a clear, comfortable view.', 'Sit naturally in a front or three-quarter view. You do not need to show your full body.')}
    <div class="split"><section>${preview()}<div class="form-row"><label for="camera-device">Camera</label><select class="select" id="camera-device" data-camera-device>${cameraOptions()}</select></div></section>
...
function calibration(): string {
  ...
  return `${heading('Calibrate', model.calibrationCandidateReady ? 'A replacement is ready.' : saved ? 'Calibration ready' : 'Set your comfortable reference.', model.calibrationCandidateReady ? 'Review the validated result before replacing your saved reference.' : saved ? 'Your validated reference is stored as derived measurements only.' : 'Sit in a comfortable posture you want to use as your reference. This is a personal comparison, not a medical standard.')}
    <div class="split"><section>${preview(false)}</section><aside class="card ${saved || model.calibrationCandidateReady ? 'soft' : ''}">
...
function statusTone(status: MonitorStatus): string {
...
function settings(): string {
  return `${heading('Settings', 'Clear controls, stored locally.', 'Changes stay on this computer. Camera and calibration changes pause monitoring before they take effect.')}
    <div class="settings-grid">
      <section class="card"><h2>General</h2><p>${trayAvailable ? 'Closing an active window keeps monitoring in the tray after a one-time explanation. Pause, Snooze, or Quit turns the camera off.' : 'A reliable system tray is unavailable, so closing the window quits and turns the camera off.'}</p><p class="fine">There is no login item or automatic camera start.</p></section>
      ...
      <div class="switch-row"><span><strong>Show preview</strong><br><span class="fine">Hiding it does not turn off an active camera.</span></span><input type="checkbox" aria-label="Show camera preview" data-setting="preview" ${model.settings.preview ? 'checked' : ''}></div><p class="fine">Defaults: Balanced sensitivity; preview shown.</p><button class="button ghost small" type="button" data-action="restore-monitoring-defaults">Restore monitoring defaults</button></section>
      <section class="card"><h2>Alerts</h2><p>Native notification API: <strong>${model.notificationCapability === 'available' ? 'Supported · delivery best effort' : model.notificationCapability === 'unavailable' ? 'Unavailable · in-app fallback active' : 'Not tested'}</strong></p><p class="fine">API support or a requested test does not prove delivery. The app never bypasses operating-system focus settings.</p><button class="button secondary" type="button" data-action="test-notification">Send test notification</button></section>
      <section class="card"><h2>Accessibility</h2><div class="switch-row"><span><strong>Reduce motion</strong><br><span class="fine">Turns off decorative movement.</span></span><input type="checkbox" aria-label="Reduce motion" data-setting="reducedMotion" ${model.settings.reducedMotion ? 'checked' : ''}></div><div class="switch-row"><span><strong>Larger status score</strong><br><span class="fine">Increases the primary dashboard status.</span></span><input type="checkbox" aria-label="Use larger status score" data-setting="largeStatus" ${model.settings.largeStatus ? 'checked' : ''}></div><p class="fine">Defaults: motion follows the system; standard status size.</p><button class="button ghost small" type="button" data-action="restore-accessibility-defaults">Restore accessibility defaults</button></section>
      <section class="card"><h2>Camera & calibration</h2><p><strong>${escapeHtml(model.cameraName)}</strong><br><span class="fine">${model.calibrationReady && model.calibratedAt ? `Calibrated ${formatDate(model.calibratedAt)}` : 'No valid calibration'}</span></p><div class="actions"><button class="button secondary" type="button" data-action="go" data-screen="positioning">Check framing</button><button class="button ghost" type="button" data-action="recalibrate">Recalibrate</button></div></section>
      <section class="card"><h2>Privacy & local data</h2><ul class="plain-list"><li><strong>config.json</strong> · preferences, selected device ID, and normalized calibration measurements</li><li><strong>history.json</strong> · minute-level summaries, retained for the selected period</li><li><strong>logs/app.log</strong> · capped, sanitized lifecycle and error codes</li><li><strong>.bak / quarantine</strong> · one recovery copy when needed</li></ul><div class="form-row"><label for="history-days">History retention</label><select id="history-days" class="select" data-setting="historyDays"><option value="0" ${model.settings.historyDays === 0 ? 'selected' : ''}>Off</option><option value="7" ${model.settings.historyDays === 7 ? 'selected' : ''}>7 days</option><option value="30" ${model.settings.historyDays === 30 ? 'selected' : ''}>30 days</option><option value="90" ${model.settings.historyDays === 90 ? 'selected' : ''}>90 days</option></select></div>
      <p class="fine">Default: 30 days.</p><div class="actions"><button class="button ghost small" type="button" data-action="restore-history-defaults">Restore history default</button><button class="button ghost small" type="button" data-action="reveal-data-folder">Reveal local data folder</button></div><div class="actions"><button class="button secondary small" type="button" data-action="confirm-delete" data-scope="history">Delete history</button><button class="button secondary small" type="button" data-action="confirm-delete" data-scope="calibration">Delete calibration</button><button class="button danger small" type="button" data-action="confirm-delete" data-scope="all">Reset all data</button></div></section>
      <section class="card"><h2>About</h2><p><strong>Open Posture ${escapeHtml(appVersion)}</strong><br><span class="fine">Build commit: ${escapeHtml(buildCommit)}</span></p><p>Open Posture is an independent, Apache-2.0 open-source wellness project. It does not diagnose, treat, prevent, or cure any condition.</p><p class="fine">Runtime networking: none · Camera recording: none · Account: none</p><p class="fine">Project source, notices, security policy, and contribution guide are available in the public repository.</p><button class="button ghost" type="button" data-action="open-source">View Apache-2.0 license (external)</button></section>
    </div>`;
}
